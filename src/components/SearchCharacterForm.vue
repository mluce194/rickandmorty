<template>
<p class="text-lg p-8 text-[#60a1c9] font-bold">Use our search form below to find information about a Rick & Morty character</p>
...
<div class="max-w-md py-4 px-8 bg-white shadow-lg rounded-lg my-20" v-for="character in result.characters.results" :key="character.id" :id="character.id" :ref="character.id" v-show="choix==character.id">
  ...
  <div>
    <h2 class="text-gray-800 text-3xl font-semibold">{{ character.name }}</h2>
        <p class="mt-2 text-gray-600"><strong>Gender</strong> : {{ character.gender }}</p>

    <p class="mt-2 text-gray-600"><strong>Origin</strong> : {{ character.origin.name }}</p>
    <p class="mt-2 text-gray-600">
    <strong>Episodes</strong> :
        <span class="mt-2 text-gray-600 after:content-['-']" v-for="episode in character.episode" :key="episode">{{ episode.episode }}</span> 
        </p>
  </div>
  <div class="flex justify-end mt-4">
    <a href="#" class="text-xl font-medium text-[#99cae9]">Status : {{ character.status }}</a>
  </div>
